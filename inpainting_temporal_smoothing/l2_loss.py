...
class TemporalLoss(nn.Module):
    ...
    def forward(self, generated, corrupted, weight_mask):
        ...
        cri = torch.nn.MSELoss(reduce=False)
        # generated = (weight_mask).float() * generated
        # corrupted = (weight_mask).float() * corrupted
        print(generated.shape)
        print(corrupted.shape)
        l2 = cri(generated, corrupted)
        lc = l2 * weight_mask
...
class temporal_inpaint(nn.Module):
    ...
    def temporal_smooth(self, ref, result, name, mask, mask_prev,threshould, per_iter_step = 1500):
        output_dir = 'output/l2_loss_big_mask/'
        ...
        original_input = result_rgb.copy()
        original_ref = ref.copy()
        prev_mask = mask_prev
        mask = mask
        mask = mask | prev_mask
        mask = mask / 255.0
        ...
        # result_rgb = torch.tensor(mask[0]).float() * result_rgb
        # ref = torch.tensor(mask[0]).float() * ref
        ...
        for i in range(per_iter_step):
            ...
            loss.backward()
            optimizer.step()
            # result_rgb.data.clamp(0,1)
            print(i, 'th contextual loss: ', loss.item())
        # print(result_rgb.shape)
        # plt.imshow(result_rgb.detach().numpy()[:, :, [2, 1, 0]])
        # plt.show()
        # result_rgb = ((1 - torch.tensor(mask[0]).float()) * original_input) + ((torch.tensor(mask[0]).float()) * result_rgb)
...
mask_prev = cv2.imread('input/256_big_mask/hallway_101_mask.png')

for i in range(102, 330):
    print('=====in ',i)
    input_img = 'input/256/hallway_' + str(i) + '_input.png'
    mask_img = 'input/256/hallway_' + str(i) + '_mask.png'
    ref_img = 'output/256_dual/rf_'+str(i-1)+'.png'
    image = cv2.imread(input_img)
    mask = cv2.imread(mask_img)

    print("asdasfasdfsdfsdf",mask)
    ...
    tempo_model.temporal_smooth(ref,result,i,mask,mask_prev, 0.002)
    mask_prev = mask.copy()
